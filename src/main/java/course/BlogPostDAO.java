...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        if(comments != null){
            ...
            if(comment != null){
                Object num_likes = comment.get("num_likes");
                comment.put("num_likes", num_likes == null ? 0 : ((Integer) num_likes).intValue() + 1);
            }
            BasicDBObject finder = new BasicDBObject("permalink", permalink);
            postsCollection.update(finder, new BasicDBObject("$set", new BasicDBObject("comments", comments)), false, false);
        }
    }
}
